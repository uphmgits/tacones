...
<div class='layout_right'>
  <div class='marketplaces_gutter'>
    <?php echo $this->htmlLink($this->owner->getHref(), $this->itemPhoto($this->owner), array('class' => 'marketplaces_gutter_photo')) ?>
    <?php echo $this->htmlLink($this->owner->getHref(), $this->owner->getTitle(), array('class' => 'marketplaces_gutter_name')) ?>

    <h4><?php echo $this->translate('Search Marketplace Items');?></h4>
...
<div class='layout_middle'>
  <h2>
    <?php echo $this->translate('%1$s\'s Marketplace Listings', $this->htmlLink($this->owner->getHref(), $this->owner->getTitle()))?>
  </h2>
  ...
  <?php if( $this->paginator->getTotalItemCount() > 0 ): ?>
    <ul class='marketplaces_entrylist'>
      <?php foreach ($this->paginator as $item): ?>
        <li>
          <h3>
            <a href='<?php echo $this->url(array('user_id' => $item->owner_id, 'marketplace_id' => $item->marketplace_id), 'marketplace_entry_view') ?>'><?php echo $item->title ?></a>
          </h3>
          ...
          <?php if ($item->comments()->getCommentCount() >0) :?>
            <a href='<?php echo $this->url(array('user_id' => $item->owner_id, 'marketplace_id' => $item->marketplace_id), 'marketplace_entry_view') ?>' class='buttonlink icon_comments'>
  ...
  <?php elseif( $this->category || $this->tag ):?>
    <div class="tip">
      <span>
        <?php echo $this->translate("%s has not posted a marketplace listing yet.", $this->owner->getTitle()) ?>
      </span>
    </div>
  <?php endif; ?>
